...
**Ground-truth comparison in the simulation study.** Since the true data-generating process is known for each of the 9 simulation scenarios, `src/simulation/ground_truth_auc.py` computes the true population-level aROC(p|x)/AUC(x) (exact closed form for the 8 Gaussian scenarios, Monte Carlo for Scenario 7's skew-normal/Student-t mixture arm) and every method -- FNN, Random Forest, naive, linear, and semiparametric-additive -- is scored against it via per-subject MSE, plus per-replicate wall-clock fit time, both aggregated by `src/postprocessing/statistics_summary.py` into a single comparison table across methods and scenarios. The Bayesian nonparametric estimator (`ROCnReg::AROC.bnp`) is not included in this Python pipeline: it has no equivalent Python implementation (it's a Dirichlet-process-mixture model specific to that package's authors), and reimplementing it from scratch was judged out of scope; its wall-clock cost is instead reported per the associated review response's allowance, via `R/aroc_bnp_timing.R` (`hpc/r_bnp_timing_array.sh`/`run_bnp_timing_array.sh`, aggregated by `src/postprocessing/aroc_bnp_timing_summary.py`/`hpc/py_bnp_summary.sh` into `output/aroc_bnp_timing/aroc_bnp_timing_summary.csv`). Measured across all 9 scenarios at both sample sizes: roughly linear in `N` (~316-434s at n=5000, ~1282-1569s at n=20000), with covariate dimension a secondary factor -- one to two orders of magnitude slower than every other method's `timing_summary.csv` entries, which is the basis for not running it at the other methods' 100-replicate scale.

**`cROC.sp` Python port validation.** `src/baselines/croc_linear_baseline.py`'s `cROC_sp` (the semiparametric linear/spline baseline `linear_reg_data_simulation.py` runs at scale for all 9 scenarios, Reviewer 1 Major Concern 2) is a Python port of `ROCnReg::cROC.sp`, not a call into the R package itself. `R/croc_sp_validation.R` and `src/baselines/croc_sp_validation.py` (run via `hpc/croc_sp_validation_array.sh`/`run_croc_sp_validation_array.sh`, diffed by `src/postprocessing/croc_sp_validation_summary.py`/`hpc/py_croc_sp_validation_summary.sh`) confirm the two agree to floating-point precision (~1e-15/1e-16 max absolute difference in AUC and in the ROC(p) curve) across all 9 scenarios and both sample sizes -- both are closed-form OLS estimators of the same model, so this is an exact-match check, not a statistical one. `R/aroc_batch_scenarios.R` (real-data-only: wrong directory layout and column names for the simulation scenarios) was never part of this comparison and doesn't need fixing as a result.
