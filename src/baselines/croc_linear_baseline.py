...
def compute_ROC(formula_h, formula_d, data_h, data_d, newdata, est_cdf, pauc, p,
    ...
    By default mu_h(x) and mu_d(x) are both evaluated at the same `newdata` rows,
    matching the paper's aROC(p|x) definition at a single shared covariate profile x.
...
def cROC_sp(formula_h, formula_d, group, tag_h, data, newdata=None,
    ...
    Parameters:
      ...
      - newdata: DataFrame on which to compute the ROC; if None, data is used. Ignored if
        newdata_h/newdata_d are given.
      - newdata_h, newdata_d: evaluate mu_h(x)/mu_d(x) at different rows (row-paired
        per subject) instead of the same shared `newdata` -- see compute_ROC.
    ...
    if newdata_h is None and newdata_d is None and newdata is None:
        newdata = data.copy()
    ...
    # Here we assume that the covariates are those appearing to the right of "~", as
    # either bare column names (e.g. "x_D_1") or patsy transform terms (e.g.
    # "bs(x_D_1, df=4)" for the semiparametric/spline formulas) -- for the latter,
    # membership is checked against patsy/statsmodels at fit time instead, since a term
    # can reference the data in ways this simple split() can't fully parse.
    rhs_vars_h = [v.strip() for v in formula_h.split("~")[1].split("+")]
    rhs_vars_d = [v.strip() for v in formula_d.split("~")[1].split("+")]
    covars = [v for v in set(rhs_vars_h + rhs_vars_d) if "(" not in v]
    required_vars = [marker, group] + covars
    for var in required_vars:
        if var not in data.columns:
            raise ValueError(f"Variable {var} is not in data")
    for frame, label in ((newdata, "newdata"), (newdata_h, "newdata_h"), (newdata_d, "newdata_d")):
        if frame is None:
            continue
        for var in covars:
            if var not in frame.columns:
                raise ValueError(f"Variable {var} is not in {label}")
    ...
    # Compute bootstrap confidence intervals (NaN-filled when B=0, i.e. no bootstrap
    # was requested -- e.g. when only the point estimate is needed, as in the
    # simulation-study ground-truth/timing comparison in linear_reg_data_simulation.py).
    alpha = (1 - ci_level) / 2
    if B > 0:
        ...
        auc_ci_lower = np.full_like(cAUC_est, np.nan)
        auc_ci_upper = np.full_like(cAUC_est, np.nan)
        roc_ci_lower = np.full_like(cROC_est, np.nan)
        roc_ci_upper = np.full_like(cROC_est, np.nan)
        coeff_h_ci_lower = coeff_h_ci_upper = pd.Series(np.nan, index=coeff_h_est.index)
        coeff_d_ci_lower = coeff_d_ci_upper = pd.Series(np.nan, index=coeff_d_est.index)
        coeff_ROC_ci_lower = coeff_ROC_ci_upper = pd.Series(np.nan, index=coeff_ROC_est.index)
...
# --- Example usage ---
# Guarded behind __main__ so importing cROC_sp/compute_ROC elsewhere (e.g.
# src/simulation/linear_reg_data_simulation.py) doesn't also try to read
# input_real_2/df_gender_2.csv and pop up a plot.
